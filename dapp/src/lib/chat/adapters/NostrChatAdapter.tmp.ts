...
export class NostrChatAdapter extends BaseChatAdapter {
  ...
  public async connect(options?: Partial<NostrChatAdapterOptions>): Promise<void> {
    ...
    try {
      ...
      this.updateConnectionStatus('connecting', { status: 'connecting', message: 'Connecting to Nostr network' });
      ...
      this.updateConnectionStatus('connected', { status: 'connected', message: 'Connected to Nostr network' });
  ...
  public async disconnect(): Promise<void> {
    ...
    try {
      ...
      this.updateConnectionStatus('disconnecting', { status: 'disconnecting', message: 'Disconnecting from Nostr network' });
      ...
      this.updateConnectionStatus('disconnected', { status: 'disconnected', message: 'Disconnected from Nostr network' });
      ...
      this.updateConnectionStatus('error', { 
        status: 'error', 
        message: `Disconnection error: ${error instanceof Error ? error.message : String(error)}` 
      });
      throw error;
    }
  }
  ...
  public async sendMessage(channelId: string, content: string, attachments?: File[]): Promise<EnhancedChatMessage> {
    ...
    try {
      ...
      if (isDirect) {
        // Extract recipient ID from channel ID
  ...
  public async getChannelDetails(channelId: string): Promise<EnhancedChatChannel> {
    ...
    try {
      // Get channel details from NostrService
      const channel = await nostrService.getTeamChannel(channelId);
      
      if (!channel) {
        throw new Error(`Channel ${channelId} not found`);
      }
      
      return this.convertNostrChannel(channel);
    } catch (error) {
