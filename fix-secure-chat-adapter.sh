...
# This script fixes corruption in SecureChatAdapter.ts
cd /Users/jono/Documents/GitHub/starcom-app/dapp

# Backup the current file
cp src/lib/chat/adapters/SecureChatAdapter.ts src/lib/chat/adapters/SecureChatAdapter.ts.bak

# Create minimal valid file
cat > src/lib/chat/adapters/SecureChatAdapter.ts << 'EOF'
/**
...
export async function createSecureChatService(): Promise<SecureChatIntegrationServiceInterface> {
  try {
    ...
    try {
      ...
      // @ts-expect-error - Using eval to avoid TypeScript errors with require
...
export class SecureChatAdapter extends BaseChatAdapter {
   ...
   */
  public async connect(_options?: Partial<EnhancedChatProviderOptions>): Promise<void> {
    if (this.isConnected()) {
      return;
    }
   ...
   */
  public async getUsers(_channelId: string): Promise<EnhancedChatUser[]> {
    if (!this.isConnected()) {
      throw new Error('Not connected to SecureChat service');
    }
  ...
  public async forwardMessage(messageId: string, sourceChannelId: string, targetChannelId: string): Promise<EnhancedChatMessage> {
    ...
    try {
      ...
      // Send as new message
      return this.sendMessage(targetChannelId, forwardedContent, originalMessage.attachments);
    } catch (error) {
