...
const logError = (message, error) => {
  ...
  if (error) {
    ...
    if (error.type && error.code) {
      // This is likely a ChatError
      console.error(chalk.red(`  Type: ${error.type}`));
      console.error(chalk.red(`  Code: ${error.code}`));
      console.error(chalk.red(`  Recoverable: ${error.recoverable ? 'Yes' : 'No'}`));
      console.error(chalk.red(`  Message: ${error.message}`));
      
      if (error.details) {
        console.error(chalk.red('  Details:'));
        console.error(chalk.red(JSON.stringify(error.details, null, 2).replace(/^/gm, '    ')));
      }
      
      if (error.context) {
        console.error(chalk.red('  Context:'));
        console.error(chalk.red(JSON.stringify(error.context, null, 2).replace(/^/gm, '    ')));
      }
    }
    
    // Always log the stack trace for debugging
    if (error.stack) {
      console.error(chalk.red('  Stack trace:'));
      console.error(chalk.red(error.stack.replace(/^/gm, '    ')));
    }
...
// Add a function to validate the test environment
async function validateTestEnvironment() {
  ...
  try {
    ...
    if (!fs.existsSync(testReportDir)) {
      fs.mkdirSync(testReportDir, { recursive: true });
      logInfo(`Created test report directory: ${testReportDir}`);
    }
    
    // Basic provider factory check
    if (typeof createChatProvider !== 'function') {
      throw new Error('ChatProviderFactory is not properly imported');
    }
    
    // Check if we can create a basic provider
    const testProvider = await createChatProvider({ 
      type: 'gun', 
      userId: 'test-validator', 
    ...
    logError('Test environment validation failed', error);
    throw error; // Re-throw to stop tests
  }
}

// Function to save the test report
function saveTestReport(testRunId, report) {
  try {
    const reportFileName = `chat-system-test-report-${testRunId}.json`;
    const reportFilePath = path.join(testReportDir, reportFileName);
    ...
    const enhancedReport = {
      ...
      duration: ((Date.now() - report.startTime) / 1000).toFixed(2),
      configuration: {
    ...
    logInfo(`Test report saved to: ${reportFilePath}`);
    return reportFilePath;
  } catch (error) {
    logError('Failed to save test report', error);
    return null;
  }
}
...
async function runTests() {
  ...
  process.on('uncaughtException', (error) => {
    ...
    try {
      logHeader('FORCE TERMINATING DUE TO UNCAUGHT EXCEPTION');
      // Save test report before exiting
      saveTestReport(testRunId, {
        success: false,
        successCount,
      ...
      console.error('Failed to save test report during fatal error', e);
    }
    
    // Force exit with error code after a short delay to allow logging
  ...
  try {
    ...
    for (const providerType of TEST_CONFIG.providers) {
      try {
        ...
        // Run provider tests with a global timeout
        await withTimeout(
          testChatProvider(providerType),
          TEST_CONFIG.timeouts.default * 2, // Double the default timeout for the entire provider suite
          `Complete test suite for ${providerType} provider`
        );
      ...
      // Add a small delay between provider tests to avoid resource contention
      await delay(2000);
    }
  } catch (error) {
...
async function testChatProvider(providerType) {
  ...
  const recordTestResult = (testName, success, duration, error = null) => {
    providerTestResults.tests[testName] = {
      success,
      duration,
      timestamp: new Date().toISOString()
    };
    ...
    if (error) {
      providerTestResults.tests[testName].error = {
        message: error.message,
        type: error.type || 'unknown',
        code: error.code || 'unknown',
        stack: error.stack
      ...
      providerTestResults.errors.push({
        testName,
        error: {
          message: error.message,
          type: error.type || 'unknown',
  ...
  try {
    ...
    const providerCreationStart = Date.now();
    ...
    try {
      ...
      const providerCreationDuration = Date.now() - providerCreationStart;
      recordTestResult('providerCreation', true, providerCreationDuration);
      
      logSuccess(`Created ${providerType} provider instances in ${providerCreationDuration}ms`);
    } catch (error) {
      const providerCreationDuration = Date.now() - providerCreationStart;
      recordTestResult('providerCreation', false, providerCreationDuration, error);
      
      logError(`Failed to create ${providerType} provider instances`, error);
      throw error;
    }
